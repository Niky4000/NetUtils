...
public class ConnectionHandler {
	...
	private void pasv() throws IOException {
		...
		try {
			ServerSocketBean serverSocketBean = getNewport(con.getAddress().getHostAddress(), this::closeServer, secureData);
			passiveServer = serverSocketBean.getServerSocket();
			newport = serverSocketBean.getPort();
			passive = true;
	...
	private ServerSocketBean getNewport(String ip, Consumer<ServerSocket> passiveServerConsumer, boolean secureData) throws IOException {
		return con.getServer().getCurrentConnectionHandler().getPort(ip, passiveServerConsumer, secureData);
	}

	private void closeServer(ServerSocket passiveServer) {
	...
	private void returnNewPort(Integer newport) {
	}
}
